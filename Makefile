...
.PHONY: rename
rename: ## Rename in all files from colaxy_localization to <new_name>: `make rename <new_name>`
	@if [ -z "$(filter-out $@,$(MAKECMDGOALS))" ]; then \
		...
		echo "\033[0;32mRenaming colaxy_localization to $$NEW_NAME in all files..."; \
		find . -type f \( -name "*.dart" -o -name "*.yaml" -o -name "*.yml" -o -name "*.md" -o -name "*.json" \) -not -path "./.dart_tool/*" -not -path "./build/*" -not -path "./.git/*" | xargs sed -i '' "s/colaxy_localization/$$NEW_NAME/g"; \
		echo "\033[0;32mRenaming lib/colaxy_localization.dart to lib/$$NEW_NAME.dart..."; \
		if [ -f "lib/colaxy_localization.dart" ]; then \
			mv "lib/colaxy_localization.dart" "lib/$$NEW_NAME.dart"; \
		fi; \
